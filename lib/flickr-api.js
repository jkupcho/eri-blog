...
const FLICKR_DEFAULT_PARAMS = `api_key=${FLICKR_API_KEY}&format=json&nojsoncallback=1&per_page=25`;
    ...
    .then(json => Promise.all(json.photoset.photo.map((photo) => {
      return fetch(`${FLICKR_API_PATH}/?${FLICKR_DEFAULT_PARAMS}&method=${PHOTO_SIZES}&photo_id=${photo.id}`)
        ...
        .then((photoSizeJson) => {
          ...
          if (sizes.filter(size => (size.label === 'Large 1600') && 
            (+size.width === 1600 && +size.height === 1200)).length > 0) {
            return {
              id: photo.id, 
              thumbnailSrc: sizes.filter(size => size.label === 'Thumbnail')[0].source,
              photoSrc: sizes.filter(size => size.label === 'Large 1600')[0].source
            };
          }
        })
      })))
